...
public class myRegister extends JFrame implements Serializable {
    ...
    public myRegister()
    {
        ...
        try {
            setContentPane(new JLabel(new ImageIcon(ImageIO.read(new File("d:\\test\\otlob11.jpg")))));
        } catch (IOException e) {
            e.printStackTrace();
        }
      ...
      /*  confirm.addActionListener(new ActionListener() {
            ...
            {
               try {
                    ObjectOutputStream bout=new ObjectOutputStream(new FileOutputStream("\"D:\test\\UsersHashMap.bin\""));
                    bout.writeChars(x);
                    bout.writeChars(y);
                    bout.writeChars(z);
                    bout.writeChars(w);
